...
export default function LiqHeatmap({ zones, currentPrice }) {
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 4, padding: '1rem 0' }}>
        {allZones.map((zone, i) => {
          ...
          const fillColor = zone.side === 'long' ? 'rgba(239,68,68,0.6)' : 'rgba(34,197,94,0.6)';

          return (
            <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
              <span style={{ fontSize: '0.7rem', color: 'var(--text-dim)', width: 35, textAlign: 'right', flexShrink: 0 }}>{zone.label}</span>
              <div style={{ flex: 1, height: 18, background: 'rgba(51,65,85,0.3)', borderRadius: 3, overflow: 'hidden' }}>
