...
export default function TradePanel({ symbol, position, onTrade }) {
  ...
  async function buy() {
    ...
    try {
      ...
      if (!data.success) addToast('error', data.message);
      else addToast('success', `Posicion abierta en ${symbol}`);
      onTrade();
  ...
  async function sell() {
    ...
    try {
      ...
      if (data.success) {
        const t = data.trade;
        const s = t.pnl >= 0 ? '+' : '';
        addToast(t.pnl >= 0 ? 'success' : 'error', `${t.symbol} vendido! PnL: ${s}$${t.pnl.toFixed(2)} (${s}${t.pnlPct.toFixed(2)}%)`);
