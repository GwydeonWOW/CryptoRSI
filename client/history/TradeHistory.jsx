          ...
          <tbody>
            {history.sort((a, b) => new Date(b.closedAt) - new Date(a.closedAt)).map((t, i) => (
              <tr key={i}>
                ...
                <td>{formatPrice(t.entryPrice)} <span style={{ color: 'var(--text-dim)', fontSize: '0.7rem' }}>(RSI:{t.rsiAtOpen?.toFixed(1) || '-'})</span></td>
                <td>{formatPrice(t.exitPrice)} <span style={{ color: 'var(--text-dim)', fontSize: '0.7rem' }}>(RSI:{t.rsiAtClose?.toFixed(1) || '-'})</span></td>
